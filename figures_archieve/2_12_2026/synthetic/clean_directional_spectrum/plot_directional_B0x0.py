...
def fit_power_law_large_k(k, Pk, k_min=900, k_max=1000, min_points=5):
    """
    Fit a power law P(k) = A * k^exponent to the large k region.
    ...
    # Filter out invalid values
    valid = (k_large > 0) & (Pk_large > 0) & np.isfinite(k_large) & np.isfinite(Pk_large)
    if valid.sum() < 3:
        # Not enough valid points, return None
        return None, None, None, None
    
    k_fit = k_large[valid]
    Pk_fit = Pk_large[valid]
...
def plot_directional_from_npz(
    ...
    if show_slope and g is not None and len(g) > 0:
...
def plot_multiple_spectra(
    ...
    for i, spec in enumerate(spectra_list):
        ...
        ax.loglog(k, Pk, linewidth=1.5, label=label, color=color)
        
        # Extract η from label and add point at k = η^{2/(m_ψ - m_φ)}
        # where m_ψ = -11/3 and m_φ = -2
        # k = η^{2/(-11/3 - (-2))} = η^{2/(-5/3)} = η^{-6/5}
        eta_match = re.search(r'\\eta=([0-9.]+)', label)
        if eta_match:
            eta = float(eta_match.group(1))
            # Skip if eta is 0 (0 raised to negative power is undefined)
            if eta > 0:
                m_psi = -11/3
                m_phi = -2
                k_vertical = 3.1415*eta ** (2 / (m_psi - m_phi))
                # Interpolate Pk at k_vertical to get y-value for the point
                if k_vertical >= min(k) and k_vertical <= max(k):
                    Pk_vertical = np.interp(k_vertical, k, Pk)
                    ax.plot(k_vertical, Pk_vertical, marker='o', markersize=8, 
                          color=color, alpha=0.7, linestyle='None')
        
        # Fit and plot power law for large k (k from 900 to 1000)
        # A_large, exponent_large, k_fit_large, Pk_fit_large = fit_power_law_large_k(k, Pk, k_min=500, k_max=1000)
        # if A_large is not None and k_fit_large is not None:
        #     # Use a lighter/dashed version of the same color for the fit
        #     fit_color_large = color
        #     # Create label with fitted exponent
        #     fit_label_large = f"$\\propto k^{{{exponent_large:.2f}}}$"
        #     ax.loglog(k_fit_large, Pk_fit_large, linestyle="--", linewidth=2.5, 
        #              alpha=1.0, color=fit_color_large, label=fit_label_large)
        #     print(f"  Fit for {label} (k: 900-1000): P(k) = {A_large:.2e} * k^{exponent_large:.3f}")
    ...
    if show_slope and len(spectra_list) > 0:
        ...
        if g is not None and len(g) > 0:
            ax.loglog(k_ref, g, linestyle="--", linewidth=3, label=r"$k^{-11/3}$", color='black')
        # Add k^{-3/2} reference line
        g_32 = _slope_guide(k_ref, Pk_ref, exponent=-3/2)
        if g_32 is not None and len(g_32) > 0:
            ax.loglog(k_ref, g_32, linestyle="--", linewidth=3, label=r"$k^{-3/2}$", color='gray')
...
def plot_log_derivative(
    ...
    ax.semilogx(k_deriv, dlogP_dlogk, linewidth=1.0, label=label, color='blue')
    
    # Add reference line for expected slope
    ax.axhline(y=reference_slope, linestyle="--", linewidth=2, 
...
def plot_multiple_log_derivatives(
    spectra_list,
    out_prefix="spectrum_comparison_deriv",
    reference_slope=-11/3,
    Lxy=None,
):
    """
    Plot log derivatives d(log P)/d(log k) for multiple spectra on the same figure.
    ...
    reference_slope : float
        Reference slope value to plot as horizontal line
    Lxy : float or None
        Box size (for labeling)
    ...
    for i, spec in enumerate(spectra_list):
        ...
        ax.semilogx(k_deriv, dlogP_dlogk, linewidth=1.0, label=label, color=color)
        
        # Extract η from label and add point at k = π * η^{2/(m_ψ - m_φ)}
        # where m_ψ = -11/3 and m_φ = -2
        # k = π * η^{2/(-11/3 - (-2))} = π * η^{2/(-5/3)} = π * η^{-6/5}
        eta_match = re.search(r'\\eta=([0-9.]+)', label)
        if eta_match:
            eta = float(eta_match.group(1))
            # Skip if eta is 0 (0 raised to negative power is undefined)
            if eta > 0:
                ...
                m_phi = -2
                k_vertical = 3.1415 * eta ** (2 / (m_psi - m_phi))
                # Interpolate dlogP_dlogk at k_vertical to get y-value for the point
                if k_vertical >= min(k_deriv) and k_vertical <= max(k_deriv):
                    dlogP_dlogk_vertical = np.interp(k_vertical, k_deriv, dlogP_dlogk)
                    ax.plot(k_vertical, dlogP_dlogk_vertical, marker='o', markersize=8, 
                          color=color, alpha=0.7, linestyle='None')
        
        k_min = min(k_min, min(k_deriv))
        k_max = max(k_max, max(k_deriv))
        deriv_min = min(deriv_min, min(dlogP_dlogk))
        deriv_max = max(deriv_max, max(dlogP_dlogk))
    
    # Add reference line for expected slope
    ax.axhline(y=reference_slope, linestyle="--", linewidth=1, 
               label=r"$k^{-11/3}$", color='black')
...
def plot_psa_spectra(
    ...
    if len(Pk_avg_list) > 0:
        ...
        if g is not None and len(g) > 0:
            ax.loglog(k, g, linestyle="--", linewidth=2, 
                     label=r"$k^{-11/3}$", color='black')
...
def main():
    ...
    for i, npz_path in enumerate(npz_paths):
        ...
        # Filter out data points where k < 2pi or k > 1000
        k_min = 2 * np.pi
        k_max = np.pi*256
        mask = (k >= k_min) & (k <= k_max)
        k = k[mask]
    ...
    print("="*60)
    print("Plotting log derivatives...")
    
    out_deriv = plot_multiple_log_derivatives(
        spectra_list,
        out_prefix="spectrum_comparison_deriv",
        reference_slope=-11/3,
        Lxy=Lxy,
