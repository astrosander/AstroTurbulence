...
for tag, cfg in cases.items():
    ...
    # Turn off sigma fluctuations, just leave mean
    # bx_mean = np.mean(bx)
    # by_mean = np.mean(by)
    # bz_mean = np.mean(bz)
    # bx = np.full_like(bx, bx_mean)
    # by = np.full_like(by, by_mean)
    # bz = np.full_like(bz, bz_mean)
    
    # print(f"bx_mean = {bx_mean:.6e}, by_mean = {by_mean:.6e}, bz_mean = {bz_mean:.6e}")
