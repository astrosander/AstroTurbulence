...
cases = {
    # "ms_10": {
    #     "bcc_path": "/home/amelnich/orcd/scratch/athena_runs/ma08_ms10/out_gpu/vtk/ms10ma08_512.mhd_bcc.00021.vtk",
    #     "w_path":   "/home/amelnich/orcd/scratch/athena_runs/ma08_ms10/out_gpu/vtk/ms10ma08_512.mhd_w.00021.vtk",
...
for tag, cfg in cases.items():
    ...
    # Subtract mean to remove mean magnetic field, keeping only fluctuations (sigma)
    bz = bz - np.mean(bz)
    # bx = bx - np.mean(bx)
    # by = by - np.mean(by)
    
    # bx = np.zeros_like(bx)
    # bz = np.full_like(bz, 12.5)
    ...
    print("P_emit", np.mean(P_emit), np.std(P_emit))
    ...
    for eta in eta_arr:
        ...
        # Save file with name based on eta value
        out_npz_eta = f"{eta}.npz"
        out_png_eta = f"{eta}.png"

        fig, axs = plt.subplots(1, 2, figsize=(12, 5), dpi=150)

        axs[0].loglog(k_centers[1:], Ek[1:])
        axs[0].set_xlabel(r"$k$")
        axs[0].set_ylabel(r"$E_u(k)$")
        axs[0].set_title(rf"{tag}  (eta={eta:.3g})")

        im = axs[1].imshow(np.log10(Pu_s.T + 1e-30), origin="lower",
        ...
        print(f"[{tag}] wrote: {out_npz_eta} and {out_png_eta}")