...
def plot_directional_from_npz(
    ...
    # Transform k to k/(2*pi)
    k_plot = k / (2 * np.pi)
    
    # Slope guide (computed on original k, then transformed)
    g = _slope_guide(k, Pk, exponent=slope_exp) if show_slope else None
    if g is not None:
        g_plot = g  # g values stay the same, only k is transformed

    # X label
    xlabel = r"$\frac{k L_{\rm box}}{2\pi}$"

    # Plot
    plt.figure(figsize=(9.0, 6.0))
    ax = plt.subplot(1, 1, 1)

    ax.loglog(k_plot, Pk, linewidth=2.5, label=label, color='blue')
    if show_slope:
        ax.loglog(k_plot, g_plot, linestyle="--", linewidth=4, label=r"slope: $-11/3$", color='black')
    
    ax.set_xlabel(xlabel)
    ax.set_ylabel(r"$P_u(k)$")
    
    if xlim is not None:
        # Transform xlim if provided
        xlim_transformed = tuple(x / (2 * np.pi) for x in xlim)
        ax.set_xlim(*xlim_transformed)
    if ylim is not None:
        ax.set_ylim(*ylim)
    else:
        ax.set_xlim(min(k_plot), max(k_plot))
...
def plot_averaged_spectrum(
    ...
    ax.loglog(k_plot, Pk, linewidth=2.5, label=label, color='blue')
    if show_slope:
        ax.loglog(k_plot, g_plot, linestyle="--", linewidth=4, label=r"$k^{-11/3}$", color='black')
...
def plot_multiple_spectra(
    ...
    spectra_list : list of tuples
        Each tuple is (k, Pk, label, color, linestyle) where:
        - k: wavenumber array
    ...
    colors_default = ['blue', 'red', 'green', 'orange', 'purple', 'brown', 'pink', 'gray']
    k_min = np.inf
    k_max = -np.inf
    Pk_min = np.inf
    Pk_max = -np.inf
    ...
    for i, spec in enumerate(spectra_list):
        if len(spec) == 3:
            ...
            k, Pk, label, color, linestyle = spec
        
        # Transform k to k/(2*pi)
        k_plot = k / (2 * np.pi)
        
        ax.loglog(k_plot, Pk, linewidth=2.5, label=label, color=color, linestyle=linestyle)
        
        # Fit and plot power law for small k (k from 10 to 100 in original units)
        # Transform k_min and k_max to original k units for fitting
        A, exponent, k_fit, Pk_fit = fit_power_law_small_k(k, Pk, k_min=10, k_max=100)
        if A is not None and k_fit is not None:
            # Transform k_fit for plotting
            k_fit_plot = k_fit / (2 * np.pi)
            ...
            # ax.loglog(k_fit_plot, Pk_fit, linestyle=":", linewidth=2.0, 
            #          alpha=0.7, color=fit_color, label=fit_label)
            print(f"  Fit for {label} (k: 10-100): P(k) = {A:.2e} * k^{exponent:.3f}")
        
        k_min = min(k_min, min(k_plot))
        k_max = max(k_max, max(k_plot))
    ...
    if show_slope and len(spectra_list) > 0:
        ...
        k_ref_plot = k_ref / (2 * np.pi)
        g = _slope_guide(k_ref, Pk_ref, exponent=slope_exp)
        ax.loglog(k_ref_plot, g, linestyle="--", linewidth=4, label=r"$k^{-11/3}$", color='black')
...
def plot_log_derivative(
    ...
    ax.semilogx(k_deriv_plot, dlogP_dlogk, linewidth=2.5, label=label, color='blue')
    ...
    ax.set_xlim(min(k_deriv_plot), max(k_deriv_plot))
    ax.legend(loc="best")
    plt.tight_layout()
...
def plot_psa_spectra(
    ...
    for i, idx in enumerate(lambda2_indices):
        if idx % 5 == 0:
            ax.loglog(k_plot, Pk_avg_list[i], linewidth=2.5, 
    ...
    if len(Pk_avg_list) > 0:
        ...
        ax.loglog(k_plot, g, linestyle="--", linewidth=4, 
                 label=r"$k^{-11/3}$", color='black')
...
def main():
    # Load all spectra
    npz_paths = [
        Path(r"D:\Downloads\2_17_2026\athena_53\spectrum_comparison\Pu_spectrum_ms1_eta0.npz"),
        Path(r"D:\Downloads\2_17_2026\athena_53\spectrum_comparison\Pu_spectrum_ms10_eta0.npz"),
        Path(r"D:\Downloads\2_17_2026\athena_53\spectrum_comparison\Pu_spectrum_ms1.npz"),
        Path(r"D:\Downloads\2_17_2026\athena_53\spectrum_comparison\Pu_spectrum_ms10.npz"),
    ]

    labels = [
        "$M_A=0.8, M_s=1, \\eta=0$",
        "$M_A=0.8, M_s=10, \\eta=0$",
        "$M_A=0.8, M_s=1, \\eta=1$",
        "$M_A=0.8, M_s=10, \\eta=1$",
    ]
    # ms=1: red tones, ms=10: blue tones
    # eta=1 is evolution of eta=0 (same color, dashed line)
    colors = ['red', 'blue', 'red', 'blue']
    linestyles = ['-', '-', '--', '--']  # solid for eta=0, dashed for eta=1
    ...
    for i, npz_path in enumerate(npz_paths):
        ...
        spectra_list.append((k, Pk, label, color, linestyle))
