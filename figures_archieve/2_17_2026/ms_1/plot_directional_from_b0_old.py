...
# Plot spectrum for each eta value without star markers
for i, eta in enumerate(np.array(eta_list, dtype=float)):
    ax.loglog(k_plot_transformed, all_Ek[i], label=rf"$\eta={eta:.3g}$", 
              color=colors[i], linewidth=2.5, alpha=0.9)
    print(k_plot_transformed)
...
P_ref_3_2 = y_norm * (k_ref_k / k_norm_k) ** (-11/3)
ax.loglog(k_ref, P_ref_3_2, '--', color='blue', linewidth=8, 
          alpha=0.7, zorder=0, label=r'$k^{-11/3}$')

ax.set_xlabel(r"$\frac{k L_{\rm box}}{2\pi}$", fontsize=28)
