...
def main():
    ...
    k_values = np.logspace(np.log10(0.0001), np.log10(100.0), 100)

    mPhi_values = [1.0]#[5.0/3.0, 1.0]
    ...
    mask_k_gt_1 = (k_values >= 1) & (k_values <= 100)
    mask_k_gt_2 = (k_values > 10) & (k_values < 50)
    
    for i, mPhi in enumerate(mPhi_values):
        slope_ref = -(mPhi + 2.0)
        # Use chi=0 curve (first chi value) for normalization
        Pdir_ref = all_Pdir[i * len(chi_values)]
        # Normalize to chi=0 curve at k=20
        ref = np.abs(Pdir_ref[i_norm]) * (k_values / k_norm)**(slope_ref)
    ...
    ref_11_3 = np.abs(Pdir_ref_11_3[i_norm]) * (k_values / k_norm)**(slope_11_3)*0.2
