...
def run_and_plot(params,
    ...
    for idx, sigma_phi2 in enumerate(sigma_phi2_list):
        ...
        # Plot Du2 curves
        label_chi = rf"$\chi={chi:.1g}$" if len(sigma_phi2_list) > 1 else r"$\frac{D_u(R;\lambda)}{2}$"
        ...
        if np.isfinite(scales["R_x"]) and (Rmin < scales["R_x"] < Rmax):
            ...
            # Plot point on curve with same color as the curve
            ax1.plot(R_x, Du2_at_Rx, 'o', color=color, markersize=8, 
        ...
        # Plot M curves
        label_M = rf"$\chi={chi:.1g}$" if len(sigma_phi2_list) > 1 else r"$\mathcal{M}(k)$"
        ...
        if np.isfinite(scales["k_x"]) and (kmin < scales["k_x"] < kmax):
            ...
            # Plot point on curve with same color as the curve
            ax2.plot(k_x, M_at_kx, 'o', color=color, markersize=8,
                    markeredgecolor='white', markeredgewidth=1.5, zorder=10)
            # Add label only for first curve
            # if idx == 0:
