...
def read_fortran_2d_array(filename):
    with open(filename, 'rb') as f:
        ...
        nx, ny = struct.unpack('ii', f.read(8)) #nx, ny, *_ = struct.unpack('iiii', f.read(16))
        f.read(4)
...
def main():
    Ma = "15"
    I = read_fortran_2d_array(f'input/Ma{Ma}/test_Kolm_Ma{Ma}B_L512_I')
    print(I)
    Q = read_fortran_2d_array(f'input/Ma{Ma}/test_Kolm_Ma{Ma}B_L512_Q')
    U = read_fortran_2d_array(f'input/Ma{Ma}/test_Kolm_Ma{Ma}B_L512_U')
    ang_file = read_fortran_2d_array(f'input/Ma{Ma}/test_Kolm_Ma{Ma}B_L512_ang')
    ...
    plt.suptitle(f'Polarization Maps - Ma={Ma}', fontsize=16, weight='bold')
