...
def main():
    ...
    # Define separated geometry slabs (fractions of Nz)
    Nz = Bx.shape[cfg.los_axis]
    emit_frac = 0.7  # emission occupies first 70% of LOS
    e0, e1 = 0, max(1, int(round(emit_frac * Nz)))
    s0, s1 = e1, Nz  # screen occupies the remainder

    # 1) Compute σΦ from the SCREEN ONLY (C=1 units)
    phi_unit = ne * Bpar  # C=1
    phi_los = np.moveaxis(phi_unit, cfg.los_axis, 0)
    dz = 1.0 / float(Nz)
    Phi_screen_map = phi_los[s0:s1, :, :].sum(axis=0) * dz
    sigmaPhi0 = float(Phi_screen_map.std())
    
    # 2) Set Faraday constant (kept at 1 unless calibrated)
    cfg.faraday_const = 1.0
    phi = cfg.faraday_const * phi_unit  # will be used only on the screen slab in maps
    ...
    # 4) Define λ² grid for sweep (uniform in λ², covering χ ∈ [0.1, 20] based on SCREEN σΦ)
    ...
    print(f"Corresponding χ (screen) range: {2*sigmaPhi0*lam2_min:.2f} to {2*sigmaPhi0*lam2_max:.2f}")
    ...
    print(f"\nComputing PSA slopes for each λ² (Separated geometry)...")
    for i, lam2 in enumerate(lam2_values):
        lam = np.sqrt(lam2)
        chi = 2.0 * sigmaPhi0 * cfg.faraday_const * lam2  # χ from SCREEN σΦ only
        
        print(f"  λ²={lam2:.3f}, χ={chi:.2f} ({i+1}/{n_points})")
        
        # Build maps for this λ (Separated geometry)
        Pi = polarized_emissivity_simple(Bx, By, gamma=gamma)
        P_map = P_map_separated(Pi, phi, lam, cfg, emit_bounds=(e0, e1), screen_bounds=(s0, s1))
        dP_map = dP_map_separated(Pi, phi, lam, cfg, emit_bounds=(e0, e1), screen_bounds=(s0, s1))
    ...
    ax1.set_title('PSA Slope of P vs χ (Separated Screen, Random-Dominated)')
    ...
    ax2.set_title('PSA Slope of dP/dλ² vs χ (Separated Screen, Random-Dominated)')
    ...
    plt.title('PSA Slopes vs χ (Separated Screen, Random-Dominated)')
